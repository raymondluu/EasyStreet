...
public class EasyStreetGUI extends JFrame implements ActionListener {
  ...
  /**
   * Advances the simulation by one frame of animation, moving each vehicle once
    ...
    for (Movable mover : my_vehicles) {
      ...
      if (mover.isAlive()) {
        ...
        if (mover.canPass(neighbors.get(new_direction), my_lights)) {
          mover.setDirection(new_direction);
          mover.setX(mover.x() + new_direction.dx());
          mover.setY(mover.y() + new_direction.dy());
        }
      } else {
      ...
      for (Movable other : my_vehicles) {
        ...
        if (mover.x() == other.x() && mover.y() == other.y()) {
  ...
  private void readVehicles(final Scanner the_input) {
    ...
    for (int line_count = 0; line_count < num_vehicles; line_count++) {
      ...
      switch (vehicle_type) {
        ...
        case 'C': // Car
          my_vehicles.add(new Car(vehicle_x, vehicle_y, Direction.valueOf(vehicle_direction)));
          break;
   ...
   */
  private void drawDebugInfo(final Graphics2D the_graphics, final Movable the_mover) {
  ...
  private void drawDebugInfo(final Graphics2D the_graphics, final int the_x, final int the_y) {
    ...
    if (my_debug_flag) {
      ...
      the_graphics.drawString("(" + the_x + ", " + the_y + ")", leftx, topy + DEBUG_OFFSET);
